...
[**打开在线 Palette Lab**](https://guanyuzooou.github.io/top-journal-palette-miner/) · [查看开放许可期刊案例](#开放许可期刊案例) · [使用命令行](#快速开始)

上传或拖入一张栅格图，调整候选颜色数量，点击色块查看其对比度，并导出 JSON 或 CSS 配色记录。实验室是纯静态、本地运行的：图像不会离开浏览器标签页。它默认使用项目原创图，而不是期刊图。

**Transfer Lab（v0.3a）** 新增可解释的第二种模式：上传参考图与目标图，填写图形类型、重要数据系列数量和可选的强调意图，即可导出确定性的颜色角色分配、映射、简化建议、可访问性提示、`transfer.json`、可读报告和 CSS 变量。它不会自动重绘或重新上色，也不会声称能从像素中推断科学重要性。
...
**v0.3.0：加入 Inspect 与 Transfer 两种 Palette Lab 模式、面板感知命令行分析，以及四个可追溯的 CC BY 4.0 期刊案例。** 工具输出供人工复核的证据，不假装仅凭像素就能完美理解视觉语义。

当前命令行工具支持 Pillow 可读取的栅格图像。`analyze_panels.py` 会根据近白色留白区域保守地提出面板候选，并生成带标注的复核图。PDF/SVG 直接输入和感知均匀颜色聚类仍在路线图中。
...
- `panel-output/panel-analysis.json`：面板候选、每个面板的颜色、参数和注意事项；
- `panel-output/panels-annotated.png`：用候选边界标注的可视复核图。

该检测器使用近白色分隔带，因此应将它视为复核助手，而不是它真正理解了图表科学布局的证明。
...
- 抗锯齿和压缩可能引入并非有意设计的颜色；
- 自动面板候选最适合带近白色留白的图，仍需要按面板进行人工复核；
- Palette Lab 分析的是浏览器缩放后的栅格预览，适合快速复核，不能代替出版尺寸下的检查。
...
- 面向明确支持的图表结构提供 preview recolouring，并复用 v0.3 的 transfer schema；
- 在 OKLab 空间进行感知颜色聚类；
- 在 Palette Lab 中加入手动面板裁剪和颜色角色覆写；
- 检测面板级颜色共现关系；
- 色觉差异模拟与对比度报告；
- 手动点击取色和复核界面；
- 可搜索的配色记录库及多格式导出。
